...
public class EnemySystem : MonoBehaviour
{
    ...
    public void SpawnZombie()
    {
        ...
        Vector3 spawnPosition = GetSpawnPosition(sectorCenterAngle);
        Transform sector = sectorSystem.GetSectorTransform(sectorIndex);

        Zombie zombie = Instantiate(zombiePrefab, spawnPosition, Quaternion.identity, sector);

        zombie.Initialize(player, playerHealth, gameManager);
        zombies.Add(zombie);
    }
